...
Phase 8 additions worth knowing:

- `src/app/api/ai/parse-query/route.ts` is the second (and last) route handler in the app besides Better Auth's catch-all — everything else stays a Server Action, and that gap is exactly what let this one ship without a guard for one review cycle (see below). It returns `200` for every *AI-mechanics* failure (bad JSON, timeout, schema mismatch): a real AI failure and an AI success are both valid outcomes of the endpoint's own job, so the client has one JSON shape to branch on (`"fallback" in data`) instead of also handling non-2xx. Authentication and a malformed body are the deliberate exception — genuine `401`/`400`s, not folded into the fallback envelope, because they aren't the AI failing.
- **Gated by `getSessionUser()` — `role === "BUYER" && status === "ACTIVE"`, else `401`.** `src/proxy.ts` only matches `/buyer|/seller|/manager`, so a route handler outside that list gets no cookie-presence check either; without its own guard this endpoint was reachable by anyone, unauthenticated, as a free proxy to a paid API. Uses `getSessionUser()`, not `requireRole()` — the latter `redirect()`s on failure, which is right for a page and wrong for a fetch caller expecting JSON back. No rate limiting on top of auth: an in-memory per-user counter is close to a no-op across Vercel's stateless function instances, and a real one needs a store this project doesn't otherwise have — a known, not a silent, gap.
- **The system prompt enumerates `COUNTRIES`/`INDUSTRIES` from `taxonomy.ts` exactly**, not just names the two fields. `parseAssetFilters` already drops any `country`/`industry` that isn't an exact match — but that only stops garbage from reaching the query, it doesn't help the model *produce* a matching string in the first place. Without the lists in the prompt, "Western Europe" or "crypto" both read the query correctly and still silently vanish downstream. The prompt also tells the model to omit rather than guess when nothing maps exactly.
- `AiSearchBar` (a colocated subcomponent inside `asset-filter-bar.tsx`, same pattern as `FilterSelect`/`PriceInput`) writes **only the keys the AI response actually returned**, and re-checks `country`/`industry` against `taxonomy.ts` client-side (belt-and-braces on the prompt fix above — a model can still slip) before writing them, showing a short note when one is dropped rather than letting a filter disappear with no explanation. A filter set manually before the AI search and not mentioned by it — Country, say — is left alone rather than cleared; "AI fills, then edit manually" and "manual filter, then AI search" both go through the same `commit`, so neither path can put the bar in a state the URL disagrees with. On "Clear filters," the parent remounts `AiSearchBar` via `key={aiResetSignal}` (same trick `PriceInput` uses) rather than pushing a reset through a prop into an effect — `react-hooks/set-state-in-effect` flags synchronous `setState` inside an effect for good reason, and `key` gets the same result without it.
- Verified locally with `ANTHROPIC_API_KEY` unset (the shipped `.env` value is an empty string, and `vercel env ls` shows no row for it in any Vercel environment either): the SDK throws resolving auth, the route's `catch` turns that into `{ fallback: true, search }`, and the page renders normally with the inline note — this is the real shape of "no key configured," not a hypothetical. Everything *around* the model call — auth, both note states, the fallback envelope, a mocked 401, Clear-filters reset — was exercised against mocked/failing responses; the model's actual extraction accuracy against the enumerated taxonomy has not yet been checked against a live call.
...
`ANTHROPIC_API_KEY` is **not set anywhere** as of Phase 8 landing (checked via `vercel env ls` — no row for it in any environment; locally `.env` ships it as an empty string, matching `.env.example`'s placeholder). Add it — on Vercel and/or locally — before relying on the live AI search; until then, both the deployed app and local dev correctly fall back to plain text search (CLAUDE.md → AI feature) rather than erroring, they just never get to try a real parse. The route's own request path was verified end to end (auth, both note states, the fallback envelope) against a mocked/failing call; the model's actual extraction accuracy has not yet been checked against a live response.
...
- **Surfaces stay true neutral** (chroma 0) — N5deal tints its field-chip backgrounds faintly blue; here the one saturated brand color carries the identity and gray stays gray. Simpler, and it's the deliberate "trochy inakshyi" (slightly different) requested when this was built. Exception: `--background` itself (not `--card`, `--muted`, or any other surface) carries a hair of chroma (`oklch(0.98 0.008 258)`, same hue as `--primary`) — added when the app still read as flat white with cards indistinguishable from the page. Cards stay pure `oklch(1 0 0)` on top, so the tint's only job is separating page canvas from card.
- **`--body-glow`**: two soft radial ellipses (indigo + violet) pinned near the top of the viewport, `body`'s `background-image`. First landed with centers just *above* the viewport (`y: -12%`), which mostly clipped off-screen and read as no visible change at all — moved to `y: ~0%` with opacity raised (light: 20/14% → 30/22%, dark: 30/22% → 38/28%) so it's actually visible on first paint instead of only provable by inspecting the CSS.
- **`CategoryBadge`** (`components/asset/asset-badges.tsx`): License/Operating Business/Stake each get a distinct tint — License reuses `--primary`, Operating Business is Tailwind `amber`, Stake is Tailwind `violet` (a quiet callback to `BrandMark`'s gradient, which drifts into the same hue). Previously all three rendered as the same gray `secondary` badge, the least scannable element on an otherwise-neutral card. Amber/violet are named directly rather than promoted to tokens, matching `MatchBadge`'s existing precedent for one-component color needs.
